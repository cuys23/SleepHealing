  ...
  Widget build(BuildContext context, WidgetRef ref) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        // backgroundColor: ThemeData.dark().scaffoldBackgroundColor,
        title: Text('Favourites', style: AppTextDecor.regular18White),
        leading: InkWell(
            onTap: () {
              context.nav.pop();
            },
                          ...
                          itemBuilder: (context, index) {
                            ...
                            return SongTile(
                              onTap: () {
                                ...
                                ref
                                    .watch(selectedAlbumProvider.notifier)
                                    .state = null;
                                ref
                                    .watch(currentPlayListProvider.notifier)
                                    .state = _.data.data!.playList!;
                              },
                              isFavourite: true,
                              track: track,
                              index: index,
                ...
                return GestureDetector(
                  onTap: () async {
                    if (isPaid) {
                      ...
                      ref.read(playerLoadingProvider.notifier).startLoading(true);
                      onTap?.call();
                      try {
                        if (index != null) {
                          await Future.delayed(const Duration(milliseconds: 500));
                          ref.read(selectedMusicIndex.notifier).state = index!;
                        }
                        await AppGLF.changeAndPlayMedia(audioHandler!, track,
                            shouldPlay: true);
                      } catch (e) {
                        debugPrint('[SongTile] play error: $e');
                      } finally {
                        ref.read(playerLoadingProvider.notifier).stopLoading(false);
                      }
                    }
                  },
                  child: Padding(
                                  ...
                                  children: [
                                    Expanded(
                                      child: Text(
                                        track.description ?? '',
                                        style: isSelected
                                            ? AppTextDecor.regular12lightGeay
                                            : AppTextDecor.regular12Gray,
                                        maxLines: 2,
                                        overflow: TextOverflow.ellipsis,
                                      ),
                                    ),
...
class FavouriteIcon extends StatelessWidget {
  const FavouriteIcon({super.key, this.isFavourite = false});
  final bool isFavourite;
