...
class FavListNotifier extends StateNotifier<ApiState<FavouriteListModel>> {
  ...
  Future<void> getFav() async {
    ...
    try {
      ...
      if (mounted) state = ApiState.loaded(data: sections);
    } catch (e) {
      if (mounted) state = ApiState.error(error: NetworkExceptions.errorText(e));
    }
  }
}
...
class FavUnFavNotifier extends StateNotifier<ApiState<String>> {
  ...
  Future<void> favUnFav() async {
    try {
      // API call শুধু background এ হবে
      await _repo.favUnFav(id: id);
      if (mounted) state = const ApiState.loaded(data: 'Success');
    } catch (e, stackTrace) {
      debugPrint(e.toString());
      debugPrint("hello $stackTrace");
      if (mounted) state = ApiState.error(error: NetworkExceptions.errorText(e));
    }
  }
